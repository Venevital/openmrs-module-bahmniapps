...
angular.module("bahmni.registration").controller("SearchPatientController", [
  "$rootScope",
  "$scope",
  "$location",
  "$window",
  ...
  function (
    ...
    const allSearchConfigs =
      appService.getAppDescriptor().getConfigValue("patientSearch") || {};
    const patientSearchResultOptions =
      allSearchConfigs.patientSearchResultOptions != null
        ? allSearchConfigs.patientSearchResultOptions
        : {};
    const ignoredIdentifiers = new Set(
      patientSearchResultOptions.ignorePatientIdentifiers || []
    );
    $scope.showAge =
      patientSearchResultOptions.showAge != null
        ? patientSearchResultOptions.showAge
        : true;
    $scope.showDOB =
      patientSearchResultOptions.showDOB != null
        ? patientSearchResultOptions.showDOB
        : false;
    $scope.extraIdentifierTypes = _.filter(
      $rootScope.patientConfiguration.identifierTypes,
      function (identifierType) {
        return (
          !identifierType.primary &&
          !ignoredIdentifiers.has(identifierType.name)
        );
      }
    );
    var patientSearchResultConfigs =
      appService.getAppDescriptor().getConfigValue("patientSearchResults") ||
      {};
    maxAttributesFromConfig = !_.isEmpty(allSearchConfigs.programAttributes)
      ? maxAttributesFromConfig - 1
      : maxAttributesFromConfig;
    ...
    var hasSearchParameters = function () {
      return (
        $scope.searchParameters.name.trim().length > 0 ||
    ...
    var searchBasedOnQueryParameters = function (offset) {
      ...
      $scope.searchParameters.addressSearchResultsConfig =
        searchParameters.addressSearchResultsConfig || "";
      $scope.searchParameters.personSearchResultsConfig =
        searchParameters.personSearchResultsConfig || "";

      $scope.searchParameters.registrationNumber =
        searchParameters.registrationNumber || "";
      if (hasSearchParameters()) {
        searching = true;
        var searchPromise = patientService
          .search(
    ...
    $scope.getProgramAttributeValues = function (result) {
      var attributeValues =
        result &&
        result.patientProgramAttributeValue &&
        result.patientProgramAttributeValue[
          $scope.programAttributesSearchConfig.field
        ];
      ...
      _.each(attributeValues, function (attr) {
        commaSeparatedAttributeValues =
          commaSeparatedAttributeValues + attr + ", ";
      });
      return commaSeparatedAttributeValues.substring(
        0,
        commaSeparatedAttributeValues.length - 2
      );
    };
    ...
    var mapProgramAttributesSearchResults = function (data) {
      if ($scope.programAttributesSearchConfig.field && data !== "Searching") {
        _.map(data.pageOfResults, function (result) {
          ...
          var arrayOfStringOfKeysValue =
            result.patientProgramAttributeValue &&
            result.patientProgramAttributeValue
              .substring(2, result.patientProgramAttributeValue.length - 2)
              .split('","');
    ...
    var setPatientIdentifierSearchConfig = function () {
      ...
      $scope.patientIdentifierSearchConfig.show =
        allSearchConfigs.searchByPatientIdentifier === undefined
          ? true
          : allSearchConfigs.searchByPatientIdentifier;
    };

    var setAddressSearchConfig = function () {
      $scope.addressSearchConfig = allSearchConfigs.address || {};
      $scope.addressSearchConfig.show =
        !_.isEmpty($scope.addressSearchConfig) &&
        !_.isEmpty($scope.addressSearchConfig.field);
      if (
        $scope.addressSearchConfig.label &&
        !$scope.addressSearchConfig.label
      ) {
    ...
    var setCustomAttributesSearchConfig = function () {
      ...
      $scope.customAttributesSearchConfig.show =
        !_.isEmpty(customAttributesSearchConfig) &&
        !_.isEmpty(customAttributesSearchConfig.fields);
    };

    var setProgramAttributesSearchConfig = function () {
      $scope.programAttributesSearchConfig =
        allSearchConfigs.programAttributes || {};
      $scope.programAttributesSearchConfig.show = !_.isEmpty(
        $scope.programAttributesSearchConfig.field
      );
    };
    ...
    var sliceExtraColumns = function () {
      ...
      _.each(orderedColumns, function (column) {
        if (
          patientSearchResultConfigs[column].fields &&
          !_.isEmpty(patientSearchResultConfigs[column].fields)
        ) {
          patientSearchResultConfigs[column].fields =
            patientSearchResultConfigs[column].fields.slice(
              patientSearchResultConfigs[column].fields,
              maxAttributesFromConfig
            );
          maxAttributesFromConfig -=
            patientSearchResultConfigs[column].fields.length;
        }
      });
    };
    ...
    var setSearchResultsConfig = function () {
      ...
      if (_.isEmpty(patientSearchResultConfigs)) {
        ...
        patientSearchResultConfigs.address = {
          fields: allSearchConfigs.address
            ? [allSearchConfigs.address.field]
            : [],
        };
        patientSearchResultConfigs.personAttributes = {
          fields: allSearchConfigs.customAttributes
            ? allSearchConfigs.customAttributes.fields
            : {},
        };
      } else {
        if (!patientSearchResultConfigs.address)
          patientSearchResultConfigs.address = {};
        if (!patientSearchResultConfigs.personAttributes)
          patientSearchResultConfigs.personAttributes = {};
      }

      if (
        patientSearchResultConfigs.address.fields &&
        !_.isEmpty(patientSearchResultConfigs.address.fields)
      ) {
        patientSearchResultConfigs.address.fields =
      ...
      $scope.personSearchResultsConfig =
        patientSearchResultConfigs.personAttributes;
    ...
    var initialize = function () {
      ...
      $scope.searchActions = appService
        .getAppDescriptor()
        .getExtensions("org.bahmni.registration.patient.search.result.action");
        ...
        .then(function (data) {
          ...
          if (data.pageOfResults.length === 1) {
            ...
            var forwardUrl =
              appService
                .getAppDescriptor()
                .getConfigValue("searchByIdForwardUrl") ||
              "/patient/{{patientUuid}}";
            $location.url(
              appService
                .getAppDescriptor()
                .formatUrl(forwardUrl, { patientUuid: patient.uuid })
            );
            ...
            $scope.noResultsMessage =
              "REGISTRATION_LABEL_COULD_NOT_FIND_PATIENT";
          }
        });
    ...
    var isUserPrivilegedForSearch = function () {
      var applicablePrivs = [
        Bahmni.Common.Constants.viewPatientsPrivilege,
        Bahmni.Common.Constants.editPatientsPrivilege,
        Bahmni.Common.Constants.addVisitsPrivilege,
        Bahmni.Common.Constants.deleteVisitsPrivilege,
      ];
      var userPrivs = _.map(
    ...
    $scope.searchPatients = function () {
      ...
      if (
        $scope.searchParameters.programAttributeFieldValue &&
        $scope.programAttributesSearchConfig.show
      ) {
        queryParams.programAttributeFieldName =
          $scope.programAttributesSearchConfig.field;
        queryParams.programAttributeFieldValue =
          $scope.searchParameters.programAttributeFieldValue;
      }
    ...
    $scope.doExtensionAction = function (extension) {
      var forwardTo = appService
        .getAppDescriptor()
        .formatUrl(extension.url, { patientUuid: $scope.selectedPatient.uuid });
